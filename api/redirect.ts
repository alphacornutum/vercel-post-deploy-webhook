...
export default async function (req: VercelRequest, res: VercelResponse) {
  ...
  const result = await fetch("https://api.vercel.com/v2/oauth/access_token", {
    ...
    body: qs.stringify({
      ...
      code: code,
      redirect_uri: `${process.env.HOST}/redirect`, // this parameter should match the Redirect URL in your integration settings on Vercel
    }),
  });
  ...
  const installation = await installationRef
    .doc(accessTokenBody.installation_id)
    .set(accessTokenBody);
